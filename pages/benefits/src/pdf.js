...
//
// Scope (round 4): overnight-only. All visits are 10-hour overnight shifts
// at the published hourly rate. Daytime support is offered by email only.
...
const SHIFT_HOURS = 10;          // hours per overnight shift
...
function buildFeeTable(numShifts, hourlyRate) {
  ...
  for (let i = 1; i <= numShifts; i++) {
    rows.push([
      ...
      // Render the subtotal once, centered vertically near the middle row,
      // to mirror Karla's layout where the running subtotal appears as a
      // single value spanning the Price column.
      i === middleRow + 1
    ...
    content: [
      ...
      },
      {
        ...
        layout: {
          hLineColor: () => '#999',
          vLineColor: () => '#999',
          hLineWidth: (i, node) => (i === node.table.body.length ? 0.5 : 0.5),
          vLineWidth: () => 0.5,
          paddingTop: () => 4,
          paddingBottom: () => 4,
          paddingLeft: () => 6,
